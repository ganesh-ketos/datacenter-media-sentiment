...
2022 and June 2026 while its sentiment reversed sign. Using two complementary
GDELT sampling frames (335,206 fulltext-mention articles and a census of
...
All raw data come from the GDELT Project \cite{leetaru2013}. We use two
frames with different tradeoffs between recall and precision:
...
The two frames agree well on volume dynamics (monthly volume correlation
$r=0.93$), and the headline frame's share of the fulltext frame rose from
...
\begin{table}[t]
\centering
\caption{Correlation between LLM net headline sentiment and
...
However, we find no evidence of a lead-lag: the cross-correlation between
...
\url{https://github.com/ketos7/datacenter-media-sentiment}. Code is MIT
licensed; datasets are derived from the GDELT Project and released under CC
BY 4.0 with attribution to GDELT \cite{gdeltweb}. All sampling and
quality-control randomness is seeded, and seeds are committed.
